...
<section>
    ...
    <?php  
        // Query per recuperare i dati della tabella TIPOLOGIE_ALLOGGIO  
        $sql = "SELECT TIPOLOGIE_ALLOGGIO.*
                FROM TIPOLOGIE_ALLOGGIO";

        $result = $conn->query($sql);

        // Generazione del codice HTML dinamico (se vi sono risultati)
        if ($result->num_rows > 0) {

            // Inizio della sezione HTML 
            echo '<div id="flex-container1-nav-section2">';

            // Output dei dati per ogni riga restituita dalla query nel formato HTML
            while($row = $result->fetch_assoc()) {
                echo '<div class="flex-item1-nav-section2" data-tipologia="' . $row['tipologia'] . '">';
                echo '<a href="#">';
                echo '<img src="imgs_loghi_homepage/' . $row['immagine'] . '" alt="' . $row['alt'] . '" class="logo">';
                echo '</a>';
                echo '<a href="#" class="tipologia">' . $row['tipologia'] . '</a>';
                echo '</div>';
            }
            echo '</div>';
        } else {
